...
const SidebarDropdown = ({ icon: Icon, label, items, isOpen, sidebarOpen }) => {
    ...
    if (!sidebarOpen) {
        return (
            <div className="relative group px-4 py-3 rounded-xl text-slate-500 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800/50 hover:text-orange-600 cursor-pointer flex justify-center">
                <Icon size={20} />
        ...
        <div className="space-y-1 font-body">
            <button
                ...
                className={`
                    ...
                    ${isExpanded ? 'bg-slate-50 dark:bg-slate-800/50 text-orange-600' : 'text-slate-500 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800/50 hover:text-orange-600'}
                `}
            >
                            ...
                            {item.isHeader ? (
                                ...
                                <NavLink
                                    ...
                                    className={({ isActive }) => `
                                        ...
                                        ${isActive
                                            ? 'text-orange-600 bg-orange-50/50 dark:bg-orange-500/10 px-3'
                                            : 'text-slate-500 dark:text-slate-400 hover:text-orange-600 hover:translate-x-1'}
                                    `}
                                >
                                    <span className="relative">
                                        {item.label}
                                        {item.badge > 0 && (
                                            <span className="absolute -top-1 -right-6 px-1.5 py-0.5 text-[8px] bg-orange-500 text-white rounded-full">
                                                {item.badge}
                                            </span>
                                        )}
                                    </span>
