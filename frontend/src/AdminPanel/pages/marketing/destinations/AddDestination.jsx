...
import { createDestination } from '../../../../services/destinationService.js';
...
export default function AddDestination() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!imageFile) return toast.error('Please upload a destination image');
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-6 animate-in fade-in duration-500 pb-10">
            <div className="flex items-center gap-4">
                ...
                <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Add Top Destination</h1>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                <form onSubmit={handleSubmit} className="lg:col-span-7 space-y-6">
                    <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-sm border border-slate-100 dark:border-slate-700 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="col-span-2 space-y-1">
                                <label className="text-xs font-bold text-slate-500 uppercase ml-1">Destination Name (Display)</label>
                                <input name="name" value={formData.name} onChange={handleInputChange} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 rounded-xl focus:ring-2 focus:ring-blue-500" placeholder="e.g. Sunny Goa" required />
                            </div>
                            <div className="space-y-1">
                                <label className="text-xs font-bold text-slate-500 uppercase ml-1">Distance (KM)</label>
                                <input type="number" name="distance" value={formData.distance} onChange={handleInputChange} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 rounded-xl focus:ring-2 focus:ring-blue-500" placeholder="e.g. 450" required />
                            </div>
                            <div className="space-y-1">
                                <label className="text-xs font-bold text-slate-500 uppercase ml-1">Travel Time</label>
                                <input name="duration" value={formData.duration} onChange={handleInputChange} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 rounded-xl focus:ring-2 focus:ring-blue-500" placeholder="e.g. 8h 30m" />
                            </div>
                        </div>

                        <div className="space-y-1 pt-2">
                            <label className="text-xs font-bold text-slate-500 uppercase ml-1">Featured Image</label>
                            <label className="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl cursor-pointer hover:bg-slate-50 transition-all overflow-hidden relative">
                                {imagePreview ? (
                                    ...
                                    <>
                                        ...
                                        <span className="text-xs font-bold text-slate-500">Upload Banner Image</span>
                                    </>
                                )}
                    ...
                    <button type="submit" disabled={loading} className="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl font-bold text-lg transition-all shadow-lg flex items-center justify-center gap-2 disabled:opacity-50">
                        ...
                        {loading ? 'Processing...' : 'Publish Destination'}
                    </button>
                </form>
                    ...
                    <div className="sticky top-6">
                        <div className="bg-slate-900 rounded-[2.5rem] p-6 shadow-xl border border-white/10 overflow-hidden">
                            <h3 className="text-white/40 text-[10px] font-bold uppercase tracking-widest mb-6 flex items-center gap-2">
                                <Sparkles size={14} className="text-amber-400" /> Card Live Preview
                            </h3>
                            ...
                            <div className="relative aspect-[4/5] w-64 mx-auto rounded-[2rem] bg-slate-800 overflow-hidden flex flex-col shadow-2xl border border-white/5">
                                ...
                                <div className="relative mt-auto p-6 space-y-2">
                                    ...
                                    <h2 className="text-xl font-black text-white leading-tight">{formData.name || 'Dest. Name'}</h2>
                                    <div className="flex items-center gap-2 text-white/70 text-[10px] font-bold">
                                        <Navigation size={10} /> {formData.name || 'Destination'}
                                    </div>
                                    <div className="pt-2 flex items-center justify-between">
                                        <span className="text-white font-black text-sm">{formData.distance || '0'} KM</span>
