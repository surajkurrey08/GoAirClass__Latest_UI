...
import { 
    ...
    MoreVertical, Edit3, Trash2, Star, 
...
export default function AllRoutes() {
    ...
    const togglePopular = async (route) => {
        try {
            const res = await updateGlobalRoute(route._id, { isPopular: !route.isPopular });
            if (res.success) {
                toast.success(route.isPopular ? 'Removed from Popular' : 'Marked as Popular');
                loadRoutes();
            }
        } catch (error) {
            toast.error(error.message);
        ...
        <div className="space-y-8 animate-in fade-in duration-700">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    ...
                    <p className="text-slate-500 font-medium mt-1">Manage global travel paths and connectivity</p>
                </div>
                <Link 
                    to="/super-admin/buses/routes/add"
                                ...
                                {routes.map((route) => (
                                    <tr key={route._id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors group">
                                        ...
                                        <td className="px-8 py-6 text-center">
                                            <span className="text-sm font-black text-slate-700 dark:text-slate-300">{route.distance}</span>
                                        </td>
                                        <td className="px-8 py-6 text-center">
